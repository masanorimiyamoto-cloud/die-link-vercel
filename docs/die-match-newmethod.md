...
> **【2026-07 更新】実寸(W×H)はホモグラフィ補正**を導入。CAL-50 の4隅から「画像→mm平面」の
> 射影変換を組み、現物輪郭を mm 平面へレクトファイしてから最小外接矩形で外寸を測る
> （`measureAndCompareDimensions` に `homography` を渡す経路）。等方 1 スカラー `avgSide/refMm`
> ではカメラの傾きで縦横に系統誤差が出る（10°で十数mm）のを直接補正する。現物が CAL-50 と
> 同一平面にある前提（抜型を平置き＋マーカーを同じ面に置く）。`calQuad` 未指定時は従来の
> `pxPerMm` 等方換算にフォールバック。輪郭ズレ(参考値)は従来どおり `pxPerMm` で算出。
