// api/drive-proxy.js
// Google Drive のファイルを alt=media でプロキシして返す。
// 必須: Vercel 環境変数 GOOGLE_SA_JSON（サービスアカウント JSON 全文）
// 安全策: dev=1（または localhost）以外では same-origin + CSRF(xcsrf Cookie と X-CSRF ヘッダ一致)を検証
...
export default async function handler(req){
  try{
    ...
    // dev=1 でも same-origin で叩かれるので、基本はOK。
    // 本番アクセスで dev=1 を付けない場合は CSRF 検証。
    ...
    if (!r.ok) {
      ...
      return json({ ok:false, error:`drive ${r.status}`, detail: detail.slice(0,500) }, r.status);
    }

    // レスポンスヘッダ整備（Content-Type は元を尊重）
